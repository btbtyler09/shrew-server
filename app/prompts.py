...
- Blank fillable forms: For blank form templates with labeled fields followed by underscored blank lines (e.g., government forms, ASME data report sheets, applications), transcribe ONLY the field labels — typically as a numbered or bulleted list (e.g., `1. Manufactured and certified by`, `2. Manufactured for`). Do NOT transcribe the blank fillable lines, do not invent placeholder values, and do not render the form as a table just to preserve layout. Field circle/marker codes (e.g., circled numbers ① ② or "(54)" markers next to fields) can be omitted unless they carry distinct meaning. If a form is FILLED IN with values, transcribe label + value pairs.
